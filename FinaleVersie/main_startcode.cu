...
*/
__device__ int find_closest_centroid_index_and_distance(bool print, int pointIndex, double &dist, double *point, double *centroids, const int numClusters, const size_t offset, const int dimension)
{
	...
	for (size_t c = 0; c < numClusters; ++c)
	{
		...
		{ // p.getSize() or dimension = N
			...
			currentdist += pow((point[i] - centroids[offset + whichCentroid + i]), 2);
			// printf("after %f\n", centroids[offset + whichCentroid + i]);
			//if (print)
				//printf("point: %d - thread: %d - value: %f\n", pointIndex, threadIdx.x, currentdist);
				//printf("point: %d - cluster: %d - dist: %f - whichCentroid: %d - point[i]: %f - centroid: %f - offset: %d - i: %d\n",
				//pointIndex, (int) c, currentdist, (int) whichCentroid, point[i], centroids[offset + whichCentroid + i], (int) offset, (int) i);
...
*/
__global__ void average_of_points_with_cluster_CUDA(
	double *avgPoint,
...
{
	int start = (blockIdx.x * blockDim.x * threadRange) + (threadIdx.x * threadRange);
	int threadIndex = blockIdx.x * blockDim.x + threadIdx.x;

	if (start < numPoints) {
		for (int point = 0; point < threadRange; ++point) {
			//if (point >= 500) return;
			int pointIndex = start + point;
			//printf("p <= numPoints: %d for %d\n", p <= numPoints, p);
			if (pointIndex < numPoints) {
				int clusterIndex = clusterOffset + pointIndex;
				if (cuClusters[clusterIndex] == centroidIndex)
				{
					//printf("cluster: %d - centroidIndex: %d - index: %d\n", cuClusters[clusterIndex], (int) centroidIndex, (int) clusterIndex);
					for (size_t j = 0; j < dimension; j++)
					{
						avgPoint[threadIndex * dimension + j] += allPoints[pointIndex * dimension + j];
						//printf("avgPoint[%d]: %f\n", threadIndex * dimension + j, avgPoint[pointIndex * dimension + j]);

					}
					numberOfPoints[threadIndex]++;
					//printf("--- number of points  %d\n", numberOfPoints[p]);
					// if (print)
					// 	printf("avgPoint[%d]: %f\n", threadIndex * dimension + 0, avgPoint[threadIndex * dimension + 0]);
				}
			}
		}
	}
...
std::vector<double> average_of_points_with_cluster(
	...
	const int dimension)
{
	std::vector<double> avgPoint(dimension, 0);
	int numberOfPoints = 0;
	for (size_t p = 0; p < numPoints; p++)
	{
		if (clusters[clusterOffset + p] == centroidIndex)
		{
			for (size_t i = 0; i < dimension; i++)
			{
				//printf("i: %d", (int) i);
...
{
	// printf("block id: %d - grid dim: %d - block dim %d\n", blockIdx.x, gridDim.x, blockDim.x );
	int start = (blockIdx.x * blockDim.x * threadRange) + (threadIdx.x * threadRange);
	...
	if (start < numPoints) {
		...
		for (int i = 0; i < threadRange; ++i)
		{
			...
			//printf("distIndex: %d + p: %d + dist: %f + cluster: %d\n", distIndex, p, dist, cuClusters[clusterOffset + p * dimension]);
		}
	}
	// if (distIndex < 32)
			   ...
			   const int dimension)
{
	...
	int threadRange = std::ceil((double) numPoints / (numBlocks * numThreads));
	//printf("numPoints: %d - numBlocks: %d - numThreads: %d - threadRange: %d\n", (int) numPoints, numBlocks, numThreads, threadRange);
	...
	while (*cuChanged)
	{
		...
		*cuChanged = false;
		...
		const size_t sizeOfDistanceSquaredSums = numBlocks * numThreads * sizeof(double);
		std::vector<double> distanceSquaredSums(numBlocks * numThreads, 0.0);
		...
		assignNewClusters<<<numBlocks,numThreads>>>(
			cuClusters,
			clusterOffset,
			cuCentroids,
			centroidOffset,
		...
		// 		cudaMemcpy(centroid.data(), &cuCentroids[centroidOffset + cluster*dimension], sizeOfCentroid, cudaMemcpyDeviceToHost);
		...
		if (*cuChanged)
		{
			...
			{
				cudaMemcpy(clusters, cuClusters, sizeOfClusters, cudaMemcpyDeviceToHost);
				std::vector<double> averagePoint = average_of_points_with_cluster(cluster, clusters, clusterOffset, points, numPoints, dimension);
				cudaMemcpy(cuClusters, clusters, sizeOfClusters, cudaMemcpyHostToDevice);
				...
				{
					centroid[coor] = averagePoint[coor];
					//printf("%f\n", cuCentroids[centroidOffset + cluster * dimension + coor]);
				}
				cudaMemcpy(&cuCentroids[centroidOffset + cluster*dimension], centroid.data(), sizeOfCentroid, cudaMemcpyHostToDevice);
			}
		}
	...
	for (int r = 0; r < repetitions; r++)
	{
		...
		*cuChangedPointer = true;
		...
		stepsPerRepetition[r] = kmeansReps(
			...
			&clusters[0],				 // CPU clusters array
			sizeOfClusters,				 // total clusters size
			cuClustersPointer,			 // CUDA clusters pointer
			numPoints * r,	 			 // clusters internal offset for this rep
			&allPoints[0],
			cuPointsPointer,
			cuChangedPointer,
			numPoints,
